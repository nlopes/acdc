...
| `bibtex` | on | cli | Resolves `cite:`/`bibliography::[]` against a `.bib` file before conversion; the pass lives in `acdc-bibtex` |
...
- **Not published**: `acdc-bibtex`, `acdc-cli`, `acdc-lint`, `acdc-lsp`, `acdc-converters-core`, `acdc-converters-html`, `acdc-converters-manpage`, `acdc-converters-markdown`, `acdc-converters-terminal`, `acdc-converters-dev`, `acdc-editor-wasm`

`acdc-cli` and `acdc-lsp` are distributed as binaries but we haven't built a pipeline to produce these as GitHub releases yet; `acdc-editor-wasm` ships via GitHub Release; the converters, `acdc-bibtex` and `acdc-converters-dev` are internal workspace members only.
