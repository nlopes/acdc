...
This keeps clear boundaries - parser, CLI, linting, LSP, and converters are independently testable. The parser has no knowledge of output formats or lint policy. Converters, linting, and the LSP depend on the parser, not the other way around. New features can be scoped to specific modules without touching everything else.
