...
mod tests {
    ...
    /// Parser warnings must flow through `update_document` into the
    /// document's LSP diagnostics as `DiagnosticSeverity::WARNING` — the
    ...
    fn test_parser_warning_becomes_lsp_diagnostic() -> Result<(), Box<dyn std::error::Error>> {
        ...
        let workspace = Workspace::new();
        let uri = "file:///warn.adoc".parse::<Uri>()?;
        // Has a doc title but first section is level 2 (===) — triggers
        // the SectionLevelOutOfSequence warning.
