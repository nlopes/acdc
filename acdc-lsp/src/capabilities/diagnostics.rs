...
use acdc_parser::{Block, Document, Error, Location, Positioning};
...
mod tests {
    ...
    /// Parse a document for tests, returning the owning `ParsedDocument`.
    /// Callers access the AST via `.document()`, which borrows from the
    /// returned wrapper and thus cannot outlive it.
    fn parse_doc(text: &str) -> Result<acdc_parser::ParsedDocument, acdc_parser::Error> {
