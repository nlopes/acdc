...
pub(crate) fn highlight_code<W: Write + ?Sized>(
    ...
    match mode {
        HighlightMode::Inline => {
            ...
            match registry.highlight(&code, &highlight_options) {
                Ok(code) => render_inline(&mut highlighted, &code, &callouts),
                Err(e) => {
                    tracing::warn!("giallo highlighting failed for language '{language}': {e}");
                    write_escaped_code_with_callouts(&mut highlighted, inlines)
                }
            }
        }
        HighlightMode::Class => {
            highlight_code_classed(&mut highlighted, &code, &callouts, language, theme_name)
        }
    }?;
    ...
    let decorated = decorate_source_lines(&highlighted, &code, &options);
    writer.write_all(decorated.as_bytes())?;
    Ok(())
}

/// Close syntax spans at line boundaries so line wrappers cannot cross them.
#[cfg(feature = "highlighting")]
...
#[cfg(feature = "highlighting")]
fn decorate_source_lines(html: &str, source: &str, options: &SourceLineOptions) -> String {
    if options.is_empty() {
        return html.to_owned();
    }

    let highlight_colour = "rgba(255,255,204,1.000)";
