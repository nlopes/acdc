...
/// Helper to get a string value from `ElementAttributes`.
///
/// Strips surrounding quotes from the value if present (parser quirk workaround).
fn get_attr_string(attrs: &ElementAttributes, name: &str) -> Option<String> {
    attrs.get(name).and_then(|v| match v {
        ...
        AttributeValue::None | AttributeValue::Bool(_) | _ => None,
    })
}
...
fn get_icon_size(attrs: &ElementAttributes) -> Option<String> {
    ...
    if let Some(size) = get_attr_string(attrs, "size") {
        return Some(size);
    }
...
pub(crate) fn write_icon<W: Write + ?Sized>(
    ...
    let span_class = match get_attr_string(attrs, "role") {
...
fn write_font_icon<W: Write + ?Sized>(
    ...
    if let Some(flip) = get_attr_string(attrs, "flip") {
        let _ = write!(classes, " fa-flip-{flip}");
    } else if let Some(rotate) = get_attr_string(attrs, "rotate") {
        let _ = write!(classes, " fa-rotate-{rotate}");
    }

    // Build title attribute
    let title_attr = get_attr_string(attrs, "title")
...
fn write_image_icon<W: Write + ?Sized>(
    ...
    let alt = get_attr_string(attrs, "alt").unwrap_or_else(|| target.to_string());

    // Build img attributes
    let mut img_attrs = format!("src=\"{iconsdir}/{target}.png\" alt=\"{alt}\"");

    if let Some(width) = get_attr_string(attrs, "width") {
        let _ = write!(img_attrs, " width=\"{width}\"");
    }

    if let Some(title) = get_attr_string(attrs, "title") {
