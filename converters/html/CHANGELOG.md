...
- `xref:target[xrefstyle=short]` sets the style for that one reference, and the
  ...
  link text.
- `xref:target[role=name]` sets `name` as the link's `class`, as in
  Asciidoctor, whether the text is automatic or given, and on references to
  other documents or unknown targets too. An empty `role=` adds no class, where
  Asciidoctor writes `class=""`.

- Repeated section titles now have distinct generated HTML IDs and
  table-of-contents links, matching Asciidoctor.
