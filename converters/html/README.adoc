...
When the HTML converter is built with the `terminal-preview` Cargo feature,
acdc can render terminal-like source blocks as selectable terminal previews.
This is an acdc-only extension; Asciidoctor does not provide a
`:terminal-preview:` attribute or equivalent built-in terminal preview feature.
...
| `:terminal-preview:`
| Enables terminal previews for terminal-like source blocks.
