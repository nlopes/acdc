...
Set `:acdc-index:` in the document header, or pass `-a acdc-index`, to generate
a catalog at an `[index]` section. The section can precede a bibliography or
sit inside a book part. Authored index content appears before the catalog.
Each catalog lists occurrences collected when it is rendered; later body
terms are excluded. The opt-in is fixed when conversion starts, so a body
unset does not disable it.

Catalog generation is an acdc extension. Without the opt-in, headings and
authored index content remain, but no catalog or occurrence anchors are
generated, matching Asciidoctor's HTML catalog policy.
...
- **Document structure**: Headers, sections, book parts and chapters, and table of contents
...
- **Cross-references**: Internal links, anchors, and IDs on explicit links, URLs, and mail links
- **Bibliographies**: Visible bracketed labels on entries and automatic citations
...
Styled `[abstract]` and `[example]` paragraphs use the same visible block
presentation as their delimited forms. Block IDs, first anchors, roles, titles,
captions, and inline formatting remain available in both the standard and
semantic backends, so cross-references can reach the rendered wrappers.
...
In book documents, numbered chapter headings and TOC entries use the active `chapter-signifier`.
An absent or unset signifier keeps the number-only form.
...
- **Callout lists** use `<ol>` instead of a table layout; a titled list uses a
...
Standalone output maps the `description` and `keywords` document attributes to
escaped `<meta>` elements. An explicitly empty attribute produces an empty
`content` value; an unset attribute produces no matching element.
...
- Reports structured converter warnings with actionable advice if a future parser variant has no HTML representation
