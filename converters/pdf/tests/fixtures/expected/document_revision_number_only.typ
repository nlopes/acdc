#set document(
  title: "Revision Number Only",
)
#set page(paper: "a4", margin: (x: 2.5cm, y: 2.5cm), fill: rgb("#ffffff"), header: context if counter(page).get().first() > 1 { align(left + horizon)[#text(fill: rgb("#374151"), weight: 500, size: 11pt)[Revision Number Only]] }, footer: text(fill: rgb("#9ca3af"), size: 9pt)[#grid(columns: (1fr, 1fr, 1fr), align(left)[], align(center)[#context counter(page).display()], align(right)[])])
...
#v(30%)
#align(center)[
#text(size: 22pt, weight: "bold")[#text("Revision Number Only")]
#v(0.4em)
#text(size: 9pt)[#text("Version 2.0")]
]
#counter(page).update(0)
]

#text("Revision number body.")
