...
#set page(paper: "a4", margin: (x: 2.5cm, y: 2.5cm), fill: rgb("#ffffff"), header: context if counter(page).get().first() > 1 { align(left + horizon)[#text(fill: rgb("#374151"), weight: 500, size: 11pt)[Ordered list starts]] }, footer: text(fill: rgb("#9ca3af"), size: 9pt)[#grid(columns: (1fr, 1fr, 1fr), align(left)[], align(center)[#context counter(page).display()], align(right)[])])
...
#align(center)[
#text(size: 22pt, weight: "bold")[#text("Ordered list starts")]
]
#v(1em)
...
#[
#set enum(numbering: (..numbers) => text(fill: rgb("#9ca3af"), numbering("1.", ..numbers.pos())), start: 5)
  + #text("Five")
  + #text("Six")
]

#heading(level: 1)[#text("Decimal boundary")] <id-5f646563696d616c5f626f756e64617279>

#[
#set enum(numbering: (..numbers) => text(fill: rgb("#9ca3af"), { let number = numbers.pos().last(); (if number < 10 { "0" } else { "" }) + str(number) + "." }), start: 9)
  + #text("Nine")
  + #text("Ten")
]
...
#heading(level: 1)[#text("Upper alpha rollover")] <id-5f75707065725f616c7068615f726f6c6c6f766572>

#[
#set enum(numbering: (..numbers) => text(fill: rgb("#9ca3af"), numbering("A.", ..numbers.pos())), start: 26)
  + #text("Z")
  + #text("AA")
]
...
#[
#set enum(numbering: (..numbers) => text(fill: rgb("#9ca3af"), numbering("i.", ..numbers.pos())), start: 4)
  + #text("Four")
  + #text("Five")
]
...
#[
#set enum(numbering: (..numbers) => text(fill: rgb("#9ca3af"), numbering("I.", ..numbers.pos())), start: 9)
  + #text("Nine")
  + #text("Ten")
]
...
#[
#set enum(numbering: (..numbers) => text(fill: rgb("#9ca3af"), { let number = numbers.pos().last(); str.from-unicode(944 + number) + "." }), start: 18)
  + #text("Final sigma")
  + #text("Sigma")
]

#heading(level: 1)[#text("Nested")] <id-5f6e6573746564>
