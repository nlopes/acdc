#set document(
  title: "Revision Date Only",
)
#set page(paper: "a4", margin: (x: 2.5cm, y: 2.5cm), fill: rgb("#ffffff"), header: context if counter(page).get().first() > 1 { align(left + horizon)[#text(fill: rgb("#374151"), weight: 500, size: 11pt)[Revision Date Only]] }, footer: text(fill: rgb("#9ca3af"), size: 9pt)[#grid(columns: (1fr, 1fr, 1fr), align(left)[], align(center)[#context counter(page).display()], align(right)[])])
...
#v(30%)
#align(center)[
#text(size: 22pt, weight: "bold")[#text("Revision Date Only")]
#v(0.4em)
#text(size: 9pt)[#text("2026-01-15")]
]
]

#set page(numbering: "1")
#counter(page).update(1)
#text("Revision date body.")
