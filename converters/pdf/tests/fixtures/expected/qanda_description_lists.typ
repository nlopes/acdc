#set document(
  title: "Q&A description lists",
)
#set page(paper: "a4", margin: (x: 2.5cm, y: 2.5cm), fill: rgb("#ffffff"), header: context if counter(page).get().first() > 1 { align(left + horizon)[#text(fill: rgb("#374151"), weight: 500, size: 11pt)[Q&A description lists]] }, footer: text(fill: rgb("#9ca3af"), size: 9pt)[#grid(columns: (1fr, 1fr, 1fr), align(left)[], align(center)[#context counter(page).display()], align(right)[])])
...
#align(center)[
#text(size: 22pt, weight: "bold")[#text("Q&A description lists")]
]
#v(1em)

#heading(level: 1)[#text("Basic questions")] <id-5f62617369635f7175657374696f6e73>

#metadata(none) <id-636f6d6d6f6e2d7175657374696f6e73>
#blocktitle[#text("Common questions")]
#[
#set enum(numbering: (..numbers) => text(fill: rgb("#9ca3af"), numbering("1.", ..numbers.pos())), spacing: 1em)
  + #block(width: 100%)[#block(width: 100%, breakable: false)[#emph[#text("What is AsciiDoc?")]#linebreak()
#text("A lightweight markup language.")]]
  + #block(width: 100%)[#block(width: 100%, breakable: false)[#emph[#text("Why use it?")]#linebreak()
#text("It produces structured documents from plain text.")]]
]

#heading(level: 1)[#text("Formatted and shared questions")] <id-5f666f726d61747465645f616e645f7368617265645f7175657374696f6e73>

#[
#set enum(numbering: (..numbers) => text(fill: rgb("#9ca3af"), numbering("1.", ..numbers.pos())), spacing: 1em)
  + #block(width: 100%)[#block(width: 100%, breakable: false)[#emph[#strong[#text("Which question is important?")]]#linebreak()
#text("The answer includes ")#strong[#text("strong")]#text(", ")#emph[#text("emphasized")]#text(", and ")#raw("code")#text(" text.")]]
  + #block(width: 100%)[#block(width: 100%, breakable: false)[#emph[#raw("Where is config.toml?")]#linebreak()
#text("In the project directory.")]]
  + #block(width: 100%)[#block(width: 100%, breakable: false)[#emph[#text("What is the short name?")]#linebreak()#emph[#text("What is the long name?")]#linebreak()
#text("Both questions share this answer.")]]
]
...
#[
#set enum(numbering: (..numbers) => text(fill: rgb("#9ca3af"), numbering("1.", ..numbers.pos())), spacing: 1em)
  + #block(width: 100%)[#block(width: 100%, breakable: false)[#emph[#text("Which steps should I follow?")]#linebreak()
#text("Start here.")]
  ...
  + #block(width: 100%)[#block(width: 100%, breakable: false)[#emph[#text("How does nested content work?")]]

#text("An attached paragraph.")

#[
#set enum(numbering: (..numbers) => text(fill: rgb("#9ca3af"), numbering("1.", ..numbers.pos())), spacing: 1em)
  + #block(width: 100%)[#block(width: 100%, breakable: false)[#emph[#text("What restarts inside the answer?")]#linebreak()
#text("The nested Q&A numbering.")]]
]

]
]

#heading(level: 1)[#text("Alignment")] <id-5f616c69676e6d656e74>

#[
#set enum(numbering: (..numbers) => text(fill: rgb("#9ca3af"), numbering("1.", ..numbers.pos())), spacing: 1em)
  + #block(width: 100%)[#block(width: 100%, breakable: false)[#align(center)[
#emph[#text("What is centered?")]#linebreak()
#text("The question and principal answer.")
]]]
]
...
#[
#set enum(numbering: (..numbers) => text(fill: rgb("#9ca3af"), numbering("1.", ..numbers.pos())), spacing: 1em)
  + #block(width: 100%)[#block(width: 100%, breakable: false)[#emph[#text("Where does numbering start?")]#linebreak()
#text("At one.")]]
  + #block(width: 100%)[#block(width: 100%, breakable: false)[#emph[#text("What comes next?")]#linebreak()
#text("Two.")]]
]
