...
#set page(paper: "a4", margin: (x: 2.5cm, y: 2.5cm), fill: rgb("#ffffff"), header: context if counter(page).get().first() > 1 { align(left + horizon)[#text(fill: rgb("#374151"), weight: 500, size: 11pt)[Partial include]] }, footer: text(fill: rgb("#9ca3af"), size: 9pt)[#grid(columns: (1fr, 1fr, 1fr), align(left)[], align(center)[#context counter(page).display()], align(right)[])])
...
#align(center)[
#text(size: 22pt, weight: "bold")[#text("Partial include")]
]
#v(1em)

#text(size: 1.25em)[#text("External: ")#link("../includes/xref_include_target.pdf#included-target")[#text("../includes/xref_include_target.pdf")]#text(" and ")#link("../includes/xref_include_target.pdf#included-target")[#text("../includes/xref_include_target.pdf")]#text(". Local: ")#context link(query(<id-696e636c756465642d746172676574>).first().location())[#text("Included target")]#text(".")]
