...
#set page(paper: "a4", margin: (x: 2.5cm, y: 2.5cm), fill: rgb("#ffffff"), header: context if counter(page).get().first() > 1 { align(left + horizon)[#text(fill: rgb("#374151"), weight: 500, size: 11pt)[Horizontal description lists]] }, footer: text(fill: rgb("#9ca3af"), size: 9pt)[#grid(columns: (1fr, 1fr, 1fr), align(left)[], align(center)[#context counter(page).display()], align(right)[])])
...
#align(center)[
#text(size: 22pt, weight: "bold")[#text("Horizontal description lists")]
]
#v(1em)

#heading(level: 1)[#text("Basic pairs")] <id-5f62617369635f7061697273>

#blocktitle[#text("Hardware terms")]
#layout(size => {
let term-width = calc.min(calc.max(0pt,
measure([#text(weight: "bold")[#text("CPU")]]).width,
measure([#text(weight: "bold")[#text("RAM")]]).width,
), size.width * 50%)
grid(columns: (term-width, 1fr), column-gutter: 20pt, row-gutter: 0.5em, align: top,
[#text(weight: "bold")[#text("CPU")]], [#text("Central processing unit.")],
[#text(weight: "bold")[#text("RAM")]], [#text("Random-access memory.")],
)
})

#heading(level: 1)[#text("Formatted and wrapping")] <id-5f666f726d61747465645f616e645f7772617070696e67>

#layout(size => {
let term-width = calc.min(calc.max(0pt,
measure([#text(weight: "bold")[#raw("API")]]).width,
measure([#text(weight: "bold")[#text("A deliberately long term that must not consume the full line")]]).width,
), size.width * 50%)
grid(columns: (term-width, 1fr), column-gutter: 20pt, row-gutter: 0.5em, align: top,
[#text(weight: "bold")[#raw("API")]], [#text("An ")#strong[#text("application programming interface")]#text(".")],
[#text(weight: "bold")[#text("A deliberately long term that must not consume the full line")]], [#text("Its description keeps enough width to wrap as normal prose.")],
)
})

#heading(level: 1)[#text("Multiple terms")] <id-5f6d756c7469706c655f7465726d73>

#layout(size => {
let term-width = calc.min(calc.max(0pt,
measure([#text(weight: "bold")[#text("Primary term")]#linebreak()#text(weight: "bold")[#text("Alternate term")]]).width,
), size.width * 50%)
grid(columns: (term-width, 1fr), column-gutter: 20pt, row-gutter: 0.5em, align: top,
[#text(weight: "bold")[#text("Primary term")]#linebreak()#text(weight: "bold")[#text("Alternate term")]], [#text("One description shared by both terms.")],
)
})

#heading(level: 1)[#text("Block descriptions")] <id-5f626c6f636b5f6465736372697074696f6e73>

#layout(size => {
let term-width = calc.min(calc.max(0pt,
measure([#text(weight: "bold")[#text("Workflow")]]).width,
), size.width * 50%)
grid(columns: (term-width, 1fr), column-gutter: 20pt, row-gutter: 0.5em, align: top,
[#text(weight: "bold")[#text("Workflow")]], [#text("An introductory line.")

#text("A continued paragraph with ")#emph[#text("formatted text")]#text(".")
