#set document(
  title: "Footnote coverage",
)
#set page(paper: "a4", margin: (x: 2.5cm, y: 2.5cm), fill: rgb("#ffffff"), header: context if counter(page).get().first() > 1 { align(left + horizon)[#text(fill: rgb("#374151"), weight: 500, size: 11pt)[Footnote coverage]] }, footer: text(fill: rgb("#9ca3af"), size: 9pt)[#grid(columns: (1fr, 1fr, 1fr), align(left)[], align(center)[#context counter(page).display()], align(right)[])])
...
#align(center)[
#text(size: 22pt, weight: "bold")[#text("Footnote coverage")]
]
#v(1em)
...
#table(columns: (1fr), align: (left + top), table.cell(x: 0, y: 0)[#text("Cell ")#counter(footnote).update(5)#footnote[#text("Cell note.")] <id-666f6f746e6f74653a63656c6c>
