#set document(
  title: "Styled block coverage",
)
#set page(paper: "a4", margin: (x: 2.5cm, y: 2.5cm), fill: rgb("#ffffff"), header: context if counter(page).get().first() > 1 { align(left + horizon)[#text(fill: rgb("#374151"), weight: 500, size: 11pt)[Styled block coverage]] }, footer: text(fill: rgb("#9ca3af"), size: 9pt)[#grid(columns: (1fr, 1fr, 1fr), align(left)[], align(center)[#context counter(page).display()], align(right)[])])
...
#blocktitle[#text("Example 1. ")#text("Example one")]
#examplebox[
#text("First example.")

]

#blocktitle[#text("Example 2. ")#text("Example two")]
#examplebox[
#text("Second example.")
...
#abstract[
#text("An abstract paragraph uses ")#strong[#text("abstract")]#text(" styling.")
]


#abstract[
#text("An abstract open block uses abstract styling.")
