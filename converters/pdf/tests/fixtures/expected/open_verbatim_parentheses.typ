...
#metadata(none) <id-6e6f726d3a636c6d756c685f6f70>
#block(width: 100%, above: 0pt, below: 0.5em)[
#text(weight: "bold")[#text("Operation")]
]

#raw(block: true, "let rs1_val = X(rs1);\nlet rs2_val = X(rs2);\nlet output : xlenbits = 0;\n\nforeach (i from 1 to xlen by 1) {\n   output = if ((rs2_val >> i) & 1)\n            then output ^ (rs1_val >> (xlen - i));\n            else output;\n}\n\nX[rd] = output")

#raw(block: true, "((visible)) (((hidden))) indexterm:[hidden] (1)")

#grid(columns: (auto, 1fr), column-gutter: 0.5em, row-gutter: 0.5em, align: (x, _) => if x == 0 { right + top } else { left + top },
[#text("(1)")], [#text("Parentheses stay literal.")],
)
