#set document(
  title: "Link display attributes",
)
#set page(paper: "a4", margin: (x: 2.5cm, y: 2.5cm), fill: rgb("#ffffff"), header: context if counter(page).get().first() > 1 { align(left + horizon)[#text(fill: rgb("#374151"), weight: 500, size: 11pt)[Link display attributes]] }, footer: text(fill: rgb("#9ca3af"), size: 9pt)[#grid(columns: (1fr, 1fr, 1fr), align(left)[], align(center)[#context counter(page).display()], align(right)[])])
...
#align(center)[
#text(size: 22pt, weight: "bold")[#text("Link display attributes")]
]
#v(1em)

#text("Before bare URL: ")#link("https://before.example/path")[#text("https://before.example/path")]

#text("Before URL macro: ")#link("https://before.example/path")[#text("https://before.example/path")]

#text("Before link macro: ")#link("https://before.example/path")[#text("https://before.example/path")]

#text("Enabled bare URL: ")#link("https://enabled.example/path")[#text("enabled.example/path")]

#text("Enabled URL macro: ")#link("https://enabled.example/path")[#text("enabled.example/path")]

#text("Enabled link macro: ")#link("https://enabled.example/path")[#text("enabled.example/path")]

#text("Explicit text: ")#link("https://enabled.example/path")[#text("example site")]

#text("FTP URL: ")#link("ftp://files.example/readme.txt")[#text("files.example/readme.txt")]

#text("Mailto macro: ")#link("mailto:person@example.com")[#text("person@example.com")]

#text("Bare email: ")#link("mailto:person@example.com")[#text("person@example.com")]

#text("False value: ")#link("http://false.example/page")[#text("false.example/page")]

#text("After unset: ")#link("https://after.example/path")[#text("https://after.example/path")]

#blocktitle[#text("Metadata-local setting")]
#link("irc://irc.example/channel")[#text("irc.example/channel")]
