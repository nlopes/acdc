#set document(
  title: "Source substitutions",
)
#set page(paper: "a4", margin: (x: 2.5cm, y: 2.5cm), fill: rgb("#ffffff"), header: context if counter(page).get().first() > 1 { align(left + horizon)[#text(fill: rgb("#374151"), weight: 500, size: 11pt)[Source substitutions]] }, footer: text(fill: rgb("#9ca3af"), size: 9pt)[#grid(columns: (1fr, 1fr, 1fr), align(left)[], align(center)[#context counter(page).display()], align(right)[])])
...
#align(center)[
#text(size: 22pt, weight: "bold")[#text("Source substitutions")]
]
#v(1em)

#blocktitle[#text("Default source substitutions")]
#raw(block: true, lang: "rust", "let value = \"{project-name}\"; // (C) ... -> *bold* (1)")

#grid(columns: (auto, 1fr), column-gutter: 0.5em, row-gutter: 0.5em, align: (x, _) => if x == 0 { right + top } else { left + top },
[#text("(1)")], [#text("Default callout.")],
)
...
#blocktitle[#text("Callouts removed")]
#raw(block: true, lang: "rust", "let value = 1; // <1>")
