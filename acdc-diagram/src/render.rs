...
struct ImagePaths {
    /// The file's name, which is what the document refers to.
    image_name: String,
    /// Its full path, needed when the image is embedded rather than linked.
    image_file: PathBuf,
}
...
fn image_node(
    ...
    let data_uri = source.doc_attr("data-uri").is_some();
    ...
    // Embedding the SVG in the page needs the file itself, not a reference.
    let embed = format == Format::Svg && apply_svg_type(&mut attributes, svg_type.as_deref())?;

    let target = if data_uri || embed {
        paths.image_file.display().to_string()
    } else {
        paths.image_name.clone()
    };
...
/// Apply `svg-type`, returning whether the image has to be embedded.
fn apply_svg_type(
    ...
    match requested {
        None | Some("static") => Ok(false),
        Some("inline") => {
            attributes.insert("inline".to_string(), String::new());
            Ok(true)
        }
        Some("interactive") => {
            attributes.insert("interactive".to_string(), String::new());
            Ok(false)
        }
