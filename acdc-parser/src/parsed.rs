...
/// produces text that was never in the source. `acdc-bibtex` is such a pass:
/// it replaces `cite:[key]` with an author and year read out of a `.bib` file,
/// text that has to become `Plain` nodes borrowing with the document's
/// lifetime.
///
...
impl ParseResult {
    ...
    /// nodes the parser produced: `acdc-bibtex` turns a `cite:[key]` into the
    /// formatted citation it stands for.
    ///
