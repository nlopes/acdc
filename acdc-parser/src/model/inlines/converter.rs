...
/// Extract plain text from `inlines` as a `String`, recursively handling
/// inline formatting nodes.
///
/// Thin wrapper around [`write_inlines`] for callers that don't already
/// have a writer. Allocates exactly one `String` for the whole subtree.
#[must_use]
