...
use acdc_parser::{
    Block, InlineMacro, InlineNode, Options, ParseResult, XrefStyle, parse, parse_inline,
};
...
#[test]
fn a_dropped_file_part_leaves_the_reference_attributes_alone() -> Result<(), Error> {
    let (target, text) = xref_of("xref:other.adoc#anchor[Text,xrefstyle=short]", true)?;
    assert_eq!((target.as_str(), text.as_str()), ("anchor", "Text"));

    let parsed = parse_inline("xref:other.adoc#anchor[xrefstyle=full]", &options(true)?)?;
    let [InlineNode::Macro(InlineMacro::CrossReference(xref))] = parsed.inlines() else {
        return Err("expected one cross-reference".into());
    };
    assert_eq!(xref.target, "anchor");
