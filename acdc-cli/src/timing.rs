...
pub(crate) fn render_summary(entries: &[TimingEntry], wall_clock: Option<Duration>) {
    if entries.is_empty() {
        return;
    }

    let mut sorted: Vec<_> = entries.iter().collect();
    sorted.sort_by_key(|entry| entry.parse + entry.convert);
    ...
    eprintln!("{}", "\u{2500}".repeat(separator_len));
    let total_parse: Duration = entries.iter().map(|entry| entry.parse).sum();
    let total_convert: Duration = entries.iter().map(|entry| entry.convert).sum();
