...
* `network` - Enable remote include parsing
...
Remote includes (`include::https://...[]`) require the parser's `network`
feature, which pulls in `ureq`. It's off by default; build with
`--features acdc-parser/network` to enable it.
