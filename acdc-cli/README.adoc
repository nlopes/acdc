...
* `terminal-preview` - Enable acdc-only selectable terminal previews in HTML output
  for terminal-like source blocks. This is not an Asciidoctor feature;
  Asciidoctor does not provide a `:terminal-preview:` attribute or equivalent
...
Build with HTML terminal previews:

[source,console]
....
cargo build --features terminal-preview
....
...
When enabled, terminal-like source blocks such as `[source,console]`,
`[source,terminal]`, or `[source,bash]` render as selectable terminal previews
in both standard and semantic HTML output. The `:terminal-preview:` document
attribute is an acdc extension and is intentionally not Asciidoctor-compatible
syntax.
...
| `:terminal-preview:`
| Enables terminal previews for terminal-like source blocks when the CLI is built with the `terminal-preview` Cargo feature.

| `:terminal-preview-cols: N`
| Sets the terminal grid width used to render previews. Defaults to `80`.

| `:terminal-preview-rows: N`
| Sets a fixed terminal grid height. When unset, acdc derives the height from the rendered terminal text plus a small padding row.

| `:terminal-cols: N`
| Alias for `:terminal-preview-cols:` when rendering HTML terminal previews.

| `:terminal-rows: N`
| Alias for `:terminal-preview-rows:` when rendering HTML terminal previews.

| `:dark-mode:`
| Uses the dark terminal preview shell colors, matching the HTML converter's dark stylesheet mode.
|===
