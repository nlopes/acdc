...
Lint `AsciiDoc` documents. The command currently provides the CLI harness,
lint registry, and lint-level flags; the lint checks are not implemented yet.
...
The available groups are `all` and `recommended-practices`.
