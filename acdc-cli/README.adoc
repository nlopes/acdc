...
A command block is a listing block carrying the `command` role and an explicit `id`; it may declare a `deps` attribute naming the ids of commands that must run first, and a `[source, <lang>]` header selects the interpreter (default `sh`).
