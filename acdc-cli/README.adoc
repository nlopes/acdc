...
`--page` accepts A3, A4, A5, Executive, Legal, Letter, or Tabloid.
...
A command block is a listing block carrying the `command` role and an explicit `id`; it may declare a `deps` attribute naming the ids of commands that must run first, and a `[source, <lang>]` header selects the interpreter (default `sh`). The `command` role and `id` are required; the source header and `deps` attribute are optional. The interpreter value is passed directly to the operating system as an executable name or path without an allowlist, so executing a document is not a sandbox boundary.
...
Commands are discovered in document order, including inside nested containers and included files, then executed in dependency order.
...
| `execute`
| no
| `acdc execute` command: run command blocks defined in AsciiDoc documents.
